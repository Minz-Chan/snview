...
public class LiveViewManager implements ClickEventUtils.OnActionListener {
	...
	public void prestoreConnectionByPosition(int pos) {
		...
		vSyncObj.width = liveviews.get(pos - 1).getSurfaceView()
				.getResolution()[0];
		...
		vSyncObj.windowTextInfo = String.valueOf(liveviews.get(pos - 1)
				.getWindowInfoText().getText());
	}
	 ...
	 * 
	 * @param isMultiMode
	 *            true，多通道预览柜式； false，单画面预览模式 ；null, 页面不作初始化
	 */
	public void setMultiMode(Boolean isMultiMode) {
		if ((isMultiMode != null && (this.isMultiMode != null) && isMultiMode
				.booleanValue() == this.isMultiMode.booleanValue())
				|| isMultiMode == null) {
			return;
		}
	...
	public LiveViewItemContainer getSelectedLiveView() {
		...
		int pos = ((currIndex % capacity) == 0) ? capacity
				: (currIndex % capacity);
	...
	public int getPositionOfIndex(int index) {
		...
		return ((index % pageCapacity) == 0) ? pageCapacity
				: (index % pageCapacity);
	}
...
//	}
	...
	public void closeAllConnection(boolean canUpdateViewAfterClosed) {
		...
		// } else {
		// connections.get(i).setDisposed(true); //
		// 若为非连接状态，则可能处于连接初始化阶段，此时将其设置为disposed状态
		// }
		//
	...
	public void closeAllConnectionExceptPos(boolean canUpdateViewAfterClosed,
			int pos) {
		int i;
		int connSize = connections.size();
	...
	public int selectLiveView(int index) {
		...
		int pageCapacity = pager.getPageCapacity();
		int pos = ((index % pageCapacity) == 0) ? pageCapacity
				: (index % pageCapacity); // 在4(或1)个LiveViewItemContainer中的位置
		int i;
		int lvSize = liveviews.size();
	...
	public void preview(int startIndex, int count) {
		if ((devicesCount - startIndex + 1 < count)
				|| startIndex > devicesCount || count < 1 || count > 4) {
			throw new IllegalArgumentException(
					"Error startIndex or count, startIndex = " + startIndex
							+ ", count = " + count);
		}
...
//			conn.reInit();
	...
	public void stopPreview(int index) {
		...
		if ((pos - 1 < connections.size())
				&& connections.get(pos - 1).isConnected()) {
