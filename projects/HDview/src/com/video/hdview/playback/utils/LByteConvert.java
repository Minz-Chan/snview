...
/**
 * @function 功能 byte数组与各种数据类型间的互转 工作项目需要在 java 和 c/c++ 之间进行 socket 通信， socket
 *           通信是以字节流或者字节包 进行的， socket发送方须将数据转换为字节流或者字节包，而接收方则将字节流和字节包再转换回
 *           相应的数据类型。如果发送方和接收方都是同种语言，则一般只涉及到字节序的调整。而对于 java 和
 *           c/c++的通信，则情况就要复杂一些，主要是因为 java 中没有 unsigned 类型，并且 java和c在
 *           某些数据类型上的长度不一致。本类就是针对这种情况，整理了 java 数据类型和网络字节流( Little-Endian)或字节包 (
 *           相当于 java 的 byte 数组 ) 之间转换方法。
 * @author 创建人 陈明珍
...
public class LByteConvert {
	...
	public static int newBytesToInt(byte b[]) {
	...
	public static long bytesToUint(byte[] array) {
		return ((long) (array[0] & 0xff)) | ((long) (array[1] & 0xff)) << 8
				| ((long) (array[2] & 0xff)) << 16
				| ((long) (array[3] & 0xff)) << 24;
	}
	...
	public static int newBytesToUint(byte[] array, int offset) {
		int value;
		value = (int) ((array[offset] & 0xFF)
