...
//	};
	...
	public void onReceive(Context context, Intent intent) {
		Log.d(TAG, TAG + "--> onReceive() is called");

		if (context == null) {
			return;
		}				
		
		SharedPreferences alarmConfig = context.getSharedPreferences(
				AlarmSettingUtils.ALARM_CONFIG, Context.MODE_PRIVATE);
		// 如果报警推送全局开关未开启，则不作处理
		if(!alarmConfig.getBoolean(AlarmSettingUtils.ALARM_CONFIG_GLOBAL_ALARM, false)){
			return;
		}
        ...
        if (wifiState != null && mobileState != null  
			...
			PushManager.startWork(context, PushConstants.LOGIN_TYPE_API_KEY,
					Utils.getMetaValue(context.getApplicationContext(),
							"api_key"));
