...
//		select_et = (EditText) findViewById(R.id.device_add_choose_et);
		...
		// et_device_add_defaultChannel = (EditText)
		// findViewById(R.id.et_device_add_defaultChannel);
		// et_device_add_channelnumber = (EditText)
...
//		select_et.setKeyListener(null);
			...
			public void onClick(View v) {
				...
				if (allEdit) {// 已经全部输入，将输入的项保存到文档中，并且验证是否可达...
					try {
						...
						Toast toast = Toast.makeText(DeviceInfoActivity.this,
						...
						editor.putString("dName",
								saveDeviceItem.getDeviceName());
						editor.putString("chSum",
								saveDeviceItem.getChannelSum());
						editor.putString("dChnl", String.valueOf(saveDeviceItem
								.getDefaultChannel()));
	...
	private void superExtendsAndHidenView() {
		...
		if (intent != null) {
			...
			if (bundle != null) {
				saveDeviceItem = (DeviceItem) bundle
