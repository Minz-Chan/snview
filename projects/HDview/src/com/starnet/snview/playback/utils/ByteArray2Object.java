...
public class ByteArray2Object {
	 ...
	 * @return
	 */
	public static Object convert2Object(Class clazz, byte byteArray[],
			int start, int arrayLength) {
		...
		if (clazz == TLV_V_PacketHeader.class) {
			...
			owspPacketHeader.setPacket_length(BByteConvert.bytesToUint(
					tempByteArray, 0));
			owspPacketHeader.setPacket_seq(LByteConvert.bytesToUint(
					tempByteArray, 4));
			...
			tlv_V_VersionInfoRequest.setVersionMajor(LByteConvert
					.bytesToUshort(tempByteArray, 0));
			tlv_V_VersionInfoRequest.setVersionMinor(LByteConvert
					.bytesToUshort(tempByteArray, 2));
			...
			tlv_V_ChannelResponse.setResult(LByteConvert.bytesToUshort(
					tempByteArray, 0));
			tlv_V_ChannelResponse.setCurrentChannel((short) LByteConvert
					.bytesToUbyte(tempByteArray, 2));
			tlv_V_ChannelResponse.setReserve((short) LByteConvert.bytesToUbyte(
					tempByteArray, 3));
			...
			tlv_V_StreamDataFormat.setVideoChannel((short) LByteConvert
					.bytesToUbyte(tempByteArray, 0)); // 视频通道号
			tlv_V_StreamDataFormat.setAudioChannel((short) LByteConvert
					.bytesToUbyte(tempByteArray, 1)); // 音频通道号
			tlv_V_StreamDataFormat.setDataType((short) LByteConvert
					.bytesToUbyte(tempByteArray, 2)); // 流数据类型,
														// 取值见StreamDataType
			tlv_V_StreamDataFormat.setReserve((short) LByteConvert
					.bytesToUbyte(tempByteArray, 3)); // 保留

			// 视频格式
			TLV_V_VideoDataFormat videoFormat = new TLV_V_VideoDataFormat();
			videoFormat.setCodecId(LByteConvert.bytesToUint(tempByteArray, 4)); // FOUR
																				// CC
																				// code，’H264’
			videoFormat.setBitrate(LByteConvert.bytesToUint(tempByteArray,
					4 + 4)); // bps
			videoFormat.setWidth(LByteConvert.bytesToUshort(tempByteArray,
					4 + 8)); // image widht
			videoFormat.setHeight(LByteConvert.bytesToUshort(tempByteArray,
					4 + 10)); // image height
			videoFormat.setFramerate((short) LByteConvert.bytesToUbyte(
					tempByteArray, 4 + 12)); // fps
			videoFormat.setColorDepth((short) LByteConvert.bytesToUbyte(
					tempByteArray, 4 + 13)); // should be 24 bits
			videoFormat.setReserve(LByteConvert.bytesToUshort(tempByteArray,
					4 + 14)); // reserve

			// 音频格式
			TLV_V_AudioDataFormat audioFormat = new TLV_V_AudioDataFormat();
			audioFormat.setSamplesPerSecond(LByteConvert.bytesToUint(
					tempByteArray, 20)); // samples per second
			// audioFormat.setSamplesPerSecond(LByteConvert.newBytesToUint(tempByteArray,
			// 20));
			audioFormat.setBitrate(LByteConvert.bytesToUint(tempByteArray,
					20 + 4)); // bps
			audioFormat.setWaveFormat(LByteConvert.bytesToUshort(tempByteArray,
					20 + 8)); // wave format, such as
								// WAVE_FORMAT_PCM,WAVE_FORMAT_MPEGLAYER3
			audioFormat.setChannelNumber(LByteConvert.bytesToUshort(
					tempByteArray, 20 + 10)); // audio channel number
			audioFormat.setBlockAlign(LByteConvert.bytesToUshort(tempByteArray,
					20 + 12)); // block alignment defined by channelSize *
								// (bitsSample/8)
			audioFormat.setBitsPerSample(LByteConvert.bytesToUshort(
					tempByteArray, 20 + 14)); // bits per sample
			audioFormat.setFrameInterval(LByteConvert.bytesToUshort(
					tempByteArray, 20 + 16)); // interval between frames, in
												// milliseconds
			audioFormat.setReserve(LByteConvert.bytesToUshort(tempByteArray,
					20 + 18)); // reserve
			...
			tlv_V_VideoFrameInfo.setChannelId((short) LByteConvert
					.bytesToUbyte(tempByteArray, 0));
			tlv_V_VideoFrameInfo.setReserve((short) LByteConvert.bytesToUbyte(
					tempByteArray, 1));
			tlv_V_VideoFrameInfo.setChecksum(LByteConvert.bytesToUshort(
					tempByteArray, 2));
			tlv_V_VideoFrameInfo.setFrameIndex(LByteConvert.bytesToUint(
					tempByteArray, 4));
			tlv_V_VideoFrameInfo.setTime(LByteConvert.bytesToUint(
					tempByteArray, 8));
			...
			tlv_V_LoginResponse.setResult(LByteConvert.bytesToUshort(
					tempByteArray, 0));
			tlv_V_LoginResponse.setReserve(LByteConvert.bytesToUshort(
					tempByteArray, 2));
			...
			tlv_V_AudioInfo.setChannelId((int) LByteConvert.bytesToUbyte(
					tempByteArray, 0));
			tlv_V_AudioInfo.setReserve((int) LByteConvert.bytesToUbyte(
					tempByteArray, 1));
			tlv_V_AudioInfo.setChecksum(LByteConvert.bytesToUshort(
					tempByteArray, 2));
			tlv_V_AudioInfo.setTime((int) LByteConvert.bytesToUint(
					tempByteArray, 4));
			...
			info.setChannelId((short) LByteConvert.bytesToUbyte(tempByteArray,
					0));
