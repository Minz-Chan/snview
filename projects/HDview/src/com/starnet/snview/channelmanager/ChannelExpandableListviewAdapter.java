 ...
 * @Description 显示扩展列表下的内容，组元素，子元素等；
 * @Modifier zhaohongxu
 * @Modify date Jul 11, 2014
...
public class ChannelExpandableListviewAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(int groupPosition, int childPosition,boolean isLastChild, View convertView, ViewGroup parent) {//加载子元素
		...
		ButtonOnclickListener bol = new ButtonOnclickListener(context,groupPosition,childPosition);//获取了所在的位置//通过第一个位置，可以获取用户的登陆用户名；通过第二个位置，可以获得是哪一个设备；groupAccountList.get(groupPosition).getDeviceList().get(childPosition);//定位到
